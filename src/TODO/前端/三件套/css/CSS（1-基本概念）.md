...
tags:
 - CSS
categories: 前端
cover: /img/index/css.png
top_img: /img/index/css.png
...
CSS 是层叠样式表（Cascading Style Sheet）的缩写，它的主要目的是为网页提供样式和时尚，CSS 提供颜色、布局、背景、字体和边框等属性。CSS 的功能允许更好的内容可及性，增强的灵活性和控制力，以及规范的表现特征。

**核心概念**

1. 选择器（Selectors）：匹配 HTML 元素（如 `p`、`.class`、`#id`、`[attr]`）。
2. 属性和值（Properties & Values）：定义样式，如 `color: red;`。
3. 盒模型（Box Model）：元素由 `content`、`padding`、`border`、`margin` 组成。
4. 布局方式：
...
5. 层叠与继承：样式按优先级（`!important` > 内联 > ID > 类 > 标签）生效，部分属性可继承。
...
2. CSS3 扩展了 CSS 的能力，提供动画、弹性布局、网格布局、响应式设计等现代 Web 开发必备功能。
3. Flexbox 和 Grid 极大简化了复杂布局的实现，而媒体查询让网页适配不同设备。