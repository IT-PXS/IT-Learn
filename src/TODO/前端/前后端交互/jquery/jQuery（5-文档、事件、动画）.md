...
### append、appendTo
```javascript
// 1.向 id 为 ul1 的 ul 下添加一个 span（最后）
// $('#ul1').append('< span > append 添加的 span </span >');
$('<span>appendTo添加的span</span>').appendTo($('#ul1'));
```

![](jQuery（5-文档、事件、动画）/1.png)

### prepend、prependTo
```javascript
// 2.向 id 为 ul1 的 ul 下添加一个 span（最前）
// $('#ul1').prepend('< span > prepend 添加的 span </span >');
$('<span>prependTo添加的span</span>').prependTo($('#ul1'));
```

![](jQuery（5-文档、事件、动画）/2.png)

### before、insertBefore
```javascript
// 3.在 id 为 ul1 的 ul 下的 li（title 为 hello）的前面添加 span
// $('#ul1').children('li[title = hello]').before('< span > before 添加的 span </span >');
$('<span>insertBefore添加的span</span>').insertBefore($('#ul1').children('li[title=hello]'));
```

![](jQuery（5-文档、事件、动画）/3.png)

### after、insertAfter
```javascript
// 4.在 id 为 ul1 的 ul 下的 li（title 为 hello）的后面添加 span
// $('#ul1').children('li[title = hello]').after('< span > after 添加的 span </span >');
$('<span>insertAfter添加的span</span>').insertAfter($('#ul1').children('li[title=hello]'));
```

![](jQuery（5-文档、事件、动画）/4.png)

### replaceWith、replaceAll
```javascript
// 5.将 id 为 ul2 的 ul 下的 li（title 为 hello）全部替换为 p
// $('#ul2').children('li[title = hello]').replaceWith('< p > replaceWith 替换的 p </p >');
$('<p>replaceWith替换的p</p>').replaceAll($('#ul2').children('li[title=hello]'));
```

![](jQuery（5-文档、事件、动画）/5.png)

### empty、remove、detach
```javascript
// 6.移除 id 为 ul2 的 ul 下的所有 li
$('#ul2').children('li').empty(); // ul2 下 li 的内容被清空
```

![](jQuery（5-文档、事件、动画）/6.png)
...
$('#addEmpButton').click(function () {
    ...
    $('<tr></tr>')
        ...
        .appendTo($('#employeeTable').children('tbody')); // 插入到默认生成的 tbody 下
    // 清除输入
    $empName.val('');
    $email.val('');
    $salary.val('');
...
| 事件 | `focus()`<br/>、`focusin()` | 当元素获得焦点时，触发 focus、focusin 事件 |
| | `blur()`<br/>、`focusout()` | 当元素失去焦点时，触发 blur、focusout 事件 |
...
1. `mouseover`/`mouseout` 进入和离开子元素会再次触发
2. `mouseenter`/`mouseleave` 进入和离开子元素不会再次触发
...
### off、unbind
```javascript
// 3.点击 btn1 解除.inner 上的所有事件监听
$('#btn1').on('click', function () {
    // $('.inner').off();
    $('.inner').unbind();
});

// 4.点击 btn2 解除.inner 上的 mouseenter 事件
...
1. `offsetX`、`offsetY`：相对于触发事件对象的坐标
2. `pageX`、`pageY`：相对于视口的坐标
3. `clientX`、`clientY`：相对于屏幕的坐标
...
// 3.点击 btn3，淡出/淡入切换，动画结束时提示“动画结束”
$('#btn3').click(function () {
...
// 2.在$(function(){})中打印 img 的宽度
$(function () {
    console.log('ready', $('#logo').width()); // ready 0
});

// 3.在 window.onload 中打印 img 的宽度
